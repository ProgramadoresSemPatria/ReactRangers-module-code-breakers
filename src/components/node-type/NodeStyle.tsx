...
export default function NodeStyle({ title, subtopics }: Readonly<NodeStyleProps>) {
  ...
  return (
    <>
      ...
      <motion.div
        className={`px-3 py-4 rounded-md shadow-md flex flex-col gap-6 text-2xl ${theme === "dark" ? 'bg-green-350 hover:bg-green-300 transition-colors text-slate-950 ' : 'bg-indigo-650 hover:bg-indigo-600 transition-colors text-white '} font-bold cursor-pointer`}
